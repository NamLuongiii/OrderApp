...
func (p *OrderPersistence) SaveOrder(order model.Order) (string, error) {
...
func (p *OrderPersistence) GetOrder(id string) (model.Order, error) {
...
func (p *OrderPersistence) GetPaginatedOrders(page, limit int) ([]model.Order, error) {
...
func (p *OrderPersistence) UpdateOrderStatus(orderId string, status model.Status) error {
	err := p.db.
		Model(&table.Order{}).
		Where("id = ?", orderId).
